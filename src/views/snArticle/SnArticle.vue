          ...
          <div class="SnArticle-2">
            <el-main>
              <el-table
                ...
                style="width: 100% "
              >
                <el-table-column label="article_id" prop="article_id">
                </el-table-column>
  ...
  methods: {
    ...
    GetAll() {
      this.$api
        .all([
          //总数
          this.$api.get("/api/SnArticle/GetCountAsync"),
          //分页
          this.$api.get(
          ...
          // 加载分类
          this.$api.get("/api/SnLabels/GetAllAsync")
        ])
        .then(
    ...
    handleDelete(index, row) {
      this.$confirm("此操作将永久删除该文件, 是否继续?", "提示", {
        ...
        .then(() => {
          this.$api({
            url: "/api/SnArticle/DeleteAsync?id=" + row.article_id,
            method: "delete"
          }).then(res => {
